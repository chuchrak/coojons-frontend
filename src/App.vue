  ...
  <div class="min-vh-100 text-light wrapper" :style="{ fontSize: globalFontSize + 'px' }">
    ...
    <div v-else class="d-flex flex-column min-vh-100 relative">
      ...
      <div class="d-flex flex-grow-1 position-relative overflow-hidden">
        ...
        <div v-if="activePayment" class="payment-overlay d-flex justify-content-center align-items-center position-fixed top-0 start-0 w-100 h-100 p-3" style="background: rgba(0,0,0,0.85); z-index: 1050;">
          <div class="card bg-panel border-0 shadow-lg rounded-4 w-100" style="max-width: 450px; overflow: hidden;">
            <div class="bg-darker p-3 p-md-4 text-center border-bottom border-secondary position-relative">
              ...
              <h5 class="text-white-50 mb-1">Do zapłaty:</h5><h1 class="fw-bold text-white mb-0 fs-2 fs-md-1">{{ activePayment.amount.toFixed(2) }} PLN</h1><p class="text-info mt-2 mb-0">{{ activePayment.title }}</p>
            </div>
            <div v-if="paymentSuccess" class="p-4 p-md-5 text-center"><h1 class="display-1 text-success mb-3">✅</h1><h4 class="text-white fw-bold">Płatność Zakończona</h4><p class="text-white-50">Transakcja została przetworzona pomyślnie.</p><button @click="closePayment" class="btn btn-success w-100 fw-bold mt-3">Wróć do systemu</button></div>
        ...
        <div class="sidebar bg-panel d-flex flex-column p-3 shadow-lg" :class="{'mobile-open': mobileMenuOpen, 'd-none d-md-flex': !mobileMenuOpen}" style="z-index: 1040;">
          ...
          <div v-if="user.role === 'student'">
            ...
            <div v-if="activeTab === 'pulpit'">
              <div class="row g-3 g-md-4 mb-4 mb-md-5">
                <div class="col-12 col-md-4"><div @click="activeTab = 'oceny'" class="card bg-panel border-0 shadow-sm h-100 p-3 p-md-4 hover-card cursor-pointer"><h5 class="fw-bold text-white mb-3 border-bottom border-secondary pb-2 fs-6 fs-md-5">Moje Oceny</h5><div class="d-flex justify-content-between align-items-center"><span class="text-white-50 small">Najnowsza ocena:</span><h3 class="text-success fw-bold mb-0 fs-4 fs-md-3">{{ myGrades.length > 0 ? myGrades[myGrades.length-1].grade : '-' }}</h3></div></div></div>
                <div class="col-12 col-md-4"><div @click="activeTab = 'plan'" class="card bg-panel border-0 shadow-sm h-100 p-3 p-md-4 hover-card cursor-pointer"><h5 class="fw-bold text-white mb-3 border-bottom border-secondary pb-2 fs-6 fs-md-5">Najbliższe zajęcia</h5><span class="text-info fw-bold d-block text-truncate small">{{ mySchedule.length > 0 ? mySchedule[0].subject : 'Brak zajęć' }}</span><small class="text-white-50 d-block text-truncate">{{ mySchedule.length > 0 ? mySchedule[0].day + ' | Sala: ' + mySchedule[0].room : 'Masz wolne!' }}</small></div></div>
                <div class="col-12 col-md-4"><div @click="activeTab = 'platnosci'" class="card bg-panel border-0 shadow-sm h-100 p-3 p-md-4 hover-card cursor-pointer" :class="unpaidCount > 0 ? 'border-start border-4 border-danger' : 'border-start border-4 border-success'"><h5 class="fw-bold text-white mb-3 border-bottom border-secondary pb-2 fs-6 fs-md-5">Stan Finansów</h5><div v-if="unpaidCount > 0"><span class="text-danger fw-bold d-block mb-1 small">Masz nieopłacone należności!</span><small class="text-white-50 d-block">Zaległe pozycje: {{ unpaidCount }}</small></div><div v-else><span class="text-success fw-bold d-block mb-1 small">Wszystko opłacone</span><small class="text-white-50 d-block">Brak zaległości finansowych</small></div></div></div>
              </div>
