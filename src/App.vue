  ...
  <div class="min-vh-100 text-light wrapper" :style="{ fontSize: globalFontSize + 'px' }">
    ...
    <div v-else class="d-flex flex-column min-vh-100 relative">
      ...
      <div class="d-flex flex-grow-1 position-relative">
        ...
        <div v-if="activePayment" class="payment-overlay d-flex justify-content-center align-items-center position-absolute top-0 start-0 w-100 h-100" style="background: rgba(0,0,0,0.85); z-index: 1060;">
          <div class="card bg-panel border-0 shadow-lg rounded-4" style="width: 450px; overflow: hidden; max-width: 95vw;">
                  ...
                  <tbody>
                    <template v-for="day in daysOfWeek" :key="day">
                      ...
                      <tr v-for="(lesson, index) in getLessonsByDay(mySchedule, day)" :key="index + day">
                        <td class="ps-3 small">{{ lesson.time }}</td><td class="text-white fw-semibold text-truncate small" style="max-width: 150px;">{{ lesson.subject }}</td><td class="small">{{ lesson.room }}</td>
                      </tr>
                    </template>
